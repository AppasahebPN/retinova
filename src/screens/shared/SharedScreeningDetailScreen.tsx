...
// Works across: ASHA, District Manager, Reviewing Doctor
// Single Source of Truth: Loaded strictly by exact Screening ID
// ============================================================
import React, { useEffect, useState } from 'react';
import {
  View, Text, StyleSheet, ScrollView, Image, ActivityIndicator,
  TouchableOpacity, Linking, Alert, Dimensions, useWindowDimensions,
} from 'react-native';
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        ...
        <View style={{ flex: 1 }}>
          <View style={styles.brandRow}>
            ...
            <View style={styles.roleBadge}>
              <Text style={styles.roleBadgeText}>Case File</Text>
            </View>
          </View>
      ...
      <View style={[
        ...
        <View>
          ...
          <Text style={[
            styles.decisionText,
            isRefer ? { color: COLORS.decisionRefer } : isScreen ? { color: COLORS.decisionScreen } : { color: COLORS.decisionRecapture }
          ]}>
            {decision}
          </Text>
        </View>
        {classification && (
          <View style={styles.decisionMetrics}>
            <Text style={styles.decisionMetricGrade}>
              {GRADE_SHORT[classification.predicted_grade] ?? '--'}
            </Text>
            <Text style={styles.decisionMetricSub}>
              {classification.g2plus_probability_calibrated !== undefined
                ? `${(classification.g2plus_probability_calibrated * 100).toFixed(1)}% Risk`
                : ''}
            </Text>
          </View>
        )}
        ...
        <View style={[styles.col, isDesktop && styles.desktopCol]}>
            ...
            {classification ? (
              <>
                <View style={styles.gradeBox}>
                  <Text style={styles.gradeBig}>{GRADE_SHORT[classification.predicted_grade]}</Text>
          ...
          <View style={styles.panel}>
            ...
            <View style={styles.evidenceGrid}>
              ...
              {lesionUrl && (
                <View style={styles.evidenceThumbCard}>
                  <Text style={[styles.evidenceThumbLabel, { color: COLORS.error }]}>CANDIDATE LESION EVIDENCE</Text>
          ...
          <View style={styles.panel}>
            ...
            {referral ? (
              <>
                <InfoRow label="Referral Status" value={referral.status} />
                ...
                <InfoRow label="Current Outcome" value={referral.action_taken ? referral.action_taken.replace('_', ' ').toUpperCase() : 'Pending review'} />
...
const styles = StyleSheet.create({
  ...
  errorTitle: { fontSize: FONTS.sizeXL, fontWeight: FONTS.weightBold, color: COLORS.error, marginBottom: SPACING.sm },
  ...
  screeningId: { fontSize: FONTS.sizeXS, color: COLORS.textMuted, marginTop: 2, fontFamily: 'monospace' },
  ...
  reportBtnText: { fontSize: FONTS.sizeXS, fontWeight: FONTS.weightBold, color: COLORS.accent },
  decisionBanner: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  ...
  decisionBannerScreen: { backgroundColor: COLORS.decisionScreenBg, borderColor: COLORS.decisionScreen },
  decisionBannerRefer: { backgroundColor: COLORS.decisionReferBg, borderColor: COLORS.decisionRefer },
  decisionBannerRecapture: { backgroundColor: COLORS.decisionRecaptureBg, borderColor: COLORS.decisionRecapture },
  decisionSub: { fontSize: FONTS.sizeXS, fontWeight: FONTS.weightBold, color: COLORS.textMuted, letterSpacing: 0.5 },
  decisionText: { fontSize: FONTS.size2XL, fontWeight: FONTS.weightBold, letterSpacing: 1 },
  decisionMetrics: { alignItems: 'flex-end' },
  decisionMetricGrade: { fontSize: FONTS.size2XL, fontWeight: FONTS.weightBold, color: COLORS.textPrimary },
  ...
  imageCaption: { fontSize: FONTS.sizeXS, color: COLORS.textMuted, textAlign: 'center', marginTop: SPACING.xs },
  gradeBox: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: COLORS.surfaceAlt,
  ...
  gradeBig: { fontSize: FONTS.size3XL, fontWeight: FONTS.weightBold, color: COLORS.textPrimary },
  gradeTitle: { fontSize: FONTS.sizeMD, fontWeight: FONTS.weightBold, color: COLORS.textPrimary },
  ...
  evidenceThumbImg: { width: '100%', height: 110, backgroundColor: '#000', borderRadius: RADIUS.sm },
  evidenceNotice: { fontSize: 9, color: COLORS.textMuted, fontStyle: 'italic', marginTop: 2 },
  doctorForm: { marginTop: SPACING.md },
  doctorFormTitle: { fontSize: FONTS.sizeSM, fontWeight: FONTS.weightBold, color: COLORS.textPrimary, letterSpacing: 0.5 },
  ...
  pillTextActive: { color: COLORS.accent, fontWeight: FONTS.weightBold },
  successBox: { backgroundColor: COLORS.successLight, padding: SPACING.sm, borderRadius: RADIUS.sm, marginBottom: SPACING.md, borderWidth: 1, borderColor: COLORS.success },
  successText: { fontSize: FONTS.sizeSM, color: COLORS.success, fontWeight: FONTS.weightSemiBold },
});
